...
screen -S ocp -p server -X stuff "source ~/.virtualenvs/ocp-cad-wrapper/bin/activate"$'\015'
screen -S ocp -p server -X stuff "~/.virtualenvs/ocp-cad-wrapper/bin/python -m ocp_vscode"$'\015'
sleep 5
screen -S ocp -p wrapper -X stuff "source ~/.virtualenvs/ocp-cad-wrapper/bin/activate"$'\015'
